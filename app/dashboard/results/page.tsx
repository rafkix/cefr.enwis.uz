...
export default function ResultPage() {
            ...
            try {
                ...
                const combined: ResultItem[] = [
                    ...rawListening.map((item: any) => ({
                        ...
                        total_questions: 35, // Multi-level formati bo'yicha [cite: 94]
                    ...
                    ...rawWriting.map((item: any) => ({
                        ...
                        created_at: item.created_at,
                        raw_score: item.raw_score || 0, 
                        total_questions: 2, // 1- va 2-topshiriqlar [cite: 87]
                        cefr_level: item.cefr_level || "—",
                        standard_score: Math.round(item.standard_score || 0)
                    }))
    ...
    const getTheme = (section: string) => {
        switch (section) {
            case 'READING': return { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200', icon: BookOpen }
            case 'LISTENING': return { bg: 'bg-purple-50', text: 'text-purple-600', border: 'border-purple-200', icon: Headphones }
            default: return { bg: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-200', icon: PenTool }
        }
    }
                                                    ...
                                                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                                                        ...
                                                        <div className="hidden md:flex flex-col items-center px-6 border-l border-slate-100 shrink-0">
                                                            <span className="text-[10px] font-bold text-slate-400 uppercase mb-1">
                                                                {item.section === 'WRITING' ? 'Topshiriqlar' : "To'g'ri javoblar"}
                                                            </span>
                                                        ...
                                                        <div className="flex items-center justify-between sm:justify-end gap-4 sm:gap-6 w-full sm:w-auto border-t sm:border-t-0 border-slate-100 pt-4 sm:pt-0">
                                                            ...
                                                            <div className={`flex items-center justify-center w-20 h-10 rounded-xl border font-bold text-sm ${levelStyle.bg} ${levelStyle.text} ${levelStyle.border}`}>
                                                                {item.cefr_level}
                                                            </div>
                                                            <Link
                    ...
                    <div className="xl:col-span-1 space-y-6">
                        <div className="sticky top-8 space-y-6">
                            ...
                            <div className="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm">
                                <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider mb-6">Bo'limlar bo'yicha faollik</h3>
                                <div className="space-y-6">
                                    {[
                                        { label: 'Reading', type: 'READING', icon: BookOpen, color: 'bg-blue-500', iconColor: 'text-blue-500' },
                                        { label: 'Listening', type: 'LISTENING', icon: Headphones, color: 'bg-purple-500', iconColor: 'text-purple-500' },
                                        { label: 'Writing', type: 'WRITING', icon: PenTool, color: 'bg-orange-500', iconColor: 'text-orange-500' }
                                    ].map((stat) => {
