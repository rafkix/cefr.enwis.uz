...
import {
  ...
  Globe, LogOut, X, Info, ShieldAlert, BadgeCheck,
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-8">
            {/* <button
                ...
                <span className="text-[10px] font-black uppercase tracking-tighter">JSON</span>
            </button> */}
          ...
          <div className="space-y-4">
            ...
            <div className="p-5 rounded-[30px] bg-slate-50/50 dark:bg-white/5 border dark:border-white/5 flex items-center gap-4 opacity-70">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Email (Asosiy)</p>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white dark:bg-[#151516] rounded-[40px] p-8 shadow-sm border dark:border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <EditableCard icon={<Globe className="text-indigo-500" />} label="Til" isEditing={isEditing}>
                {isEditing ? (
                  <select
                    className="w-full bg-transparent border-none outline-none font-bold text-[14px] dark:text-white"
                  ...
                  <p className="font-bold text-[14px] dark:text-white uppercase">{formData.language}</p>
                )}
              </EditableCard>
              ...
              <EditableCard icon={<SmartphoneNfc className="text-blue-500" />} label="Vaqt mintaqasi" isEditing={false}>
                <p className="font-bold text-[14px] dark:text-white truncate">{formData.timezone}</p>
              </EditableCard>

              <EditableCard icon={<Check className="text-green-500" />} label="BIO" isEditing={isEditing} isFullWidth>
                <textarea
          ...
          <div className="bg-white dark:bg-[#151516] rounded-[40px] p-8 shadow-sm border dark:border-white/5">
            ...
            <div className="space-y-3">
              {sessions.map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center justify-between p-4 rounded-[28px] bg-slate-50 dark:bg-white/5 border dark:border-white/5 gap-3">
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                      <div className={`p-4 rounded-2xl shrink-0 bg-white dark:bg-white/10 text-slate-400`}>
                        {s.user_agent.toLowerCase().includes("mobile") ? <Smartphone size={20} /> : <Globe size={20} />}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                          <p className="font-bold text-[14px] dark:text-white truncate">{s.ip_address}</p>
                          {s.is_revoked && <span className="text-[8px] bg-red-500/10 text-red-500 px-1.5 py-0.5 rounded-md uppercase font-black">Revoked</span>}
                        </div>
                        <p className="text-[11px] text-slate-400 font-medium truncate opacity-70">{s.user_agent}</p>
                      </div>
                    </div>
                    {!s.is_revoked && (
                      <button
...
const EditableCard = ({ icon, label, children, isEditing, isFullWidth }: any) => (
  <div
    className={`p-5 rounded-[30px] border transition-all flex items-center gap-4 ${
      isEditing ? "bg-blue-500/5 border-blue-500/30 ring-2 ring-blue-500/10" : "bg-white dark:bg-white/5 border-transparent"
