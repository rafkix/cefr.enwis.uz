...
import { userService } from '@/lib/api/user' // Siz bergan yangi servis
...
export default function ProfilePage() {
  ...
  // Form data interfeysga moslandi
  ...
  // 1. MA'LUMOTLARNI YUKLASH (Sizning userService orqali)
  ...
  // 4. KONTAKT QO'SHISH (Telefon raqam)
        ...
        <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-8">
          <div className="bg-white dark:bg-[#151516] rounded-[40px] p-8 shadow-sm border dark:border-white/5 text-center">
            ...
            <div className="flex gap-2">
              {isEditing ? (
                <>
                  <button
                    ...
                    disabled={loading}
                    className="flex-1 bg-green-500 text-white py-4 rounded-2xl font-black text-[10px] tracking-widest uppercase flex items-center justify-center gap-2 transition-all"
                  >
                    {loading ? (
                  ...
                  <button
                    ...
                    className="p-4 bg-slate-100 dark:bg-white/5 text-slate-500 rounded-2xl"
                  >
                    <X size={20} />
                  </button>
                </>
                ...
                <button
                  ...
                  className="flex-1 bg-slate-50 dark:bg-white/5 py-4 rounded-2xl font-black text-[10px] tracking-widest uppercase flex items-center justify-center gap-2 border dark:border-white/5 dark:text-white"
                >
              ...
              <button
                onClick={logout}
                className="p-4 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-2xl border border-red-100 dark:border-red-500/20"
              >
                <LogOut size={20} />
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-[#151516] rounded-[32px] p-6 border dark:border-white/5 space-y-4">
            ...
            <StatusRow
              ...
              icon={
                <div
                  className={`w-3 h-3 rounded-full ${phoneContact?.is_verified ? 'bg-green-500 shadow-lg shadow-green-500/50' : 'bg-slate-300'}`}
                />
              }
            />
          </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white dark:bg-[#151516] rounded-[40px] p-8 shadow-sm border dark:border-white/5">
            <div className="flex justify-between items-center mb-8">
              ...
              {isEditing && (
                <span className="text-[10px] font-black text-orange-500 bg-orange-500/10 px-3 py-1 rounded-full uppercase animate-pulse">
                  Tahrirlash
                </span>
              )}
            </div>
                ...
                <input
                  ...
                  value={
                    isEditing
                      ? formData.birth_date
                      : formData.birth_date
                        ? new Date(formData.birth_date).toLocaleDateString('uz-UZ')
                        : '—'
                  }
                  onChange={(e) =>
          ...
          <div className="bg-white dark:bg-[#151516] rounded-[40px] p-8 shadow-sm border dark:border-white/5">
            ...
            <div className="space-y-3">
              {sessions.map((s) => (
                <div
                  key={s.id}
                  className="flex items-center justify-between p-4 sm:p-5 rounded-[24px] sm:rounded-[28px] bg-slate-50 dark:bg-white/5 border dark:border-white/5 gap-3"
                >
                  <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                    {/* Icon qismi - mobil uchun biroz kichraytirildi */}
                    <div
                      className={`p-3 sm:p-4 rounded-2xl shrink-0 ${s.is_current ? 'bg-blue-500 text-white' : 'bg-white dark:bg-white/10 text-slate-400'}`}
                    >
                      {s.user_agent.toLowerCase().includes('mobile') ? (
                        <Smartphone size={18} className="sm:w-[20px]" />
                      ) : (
                        <Globe size={18} className="sm:w-[20px]" />
                      )}
                    </div>
