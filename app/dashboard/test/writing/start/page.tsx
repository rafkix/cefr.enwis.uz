...
const TaskTextArea = memo(function TaskTextArea({
  ...
  // restore/sync from parent (initial load yoki boshqa tabdan restore)
  useEffect(() => {
    setLocalValue(value)
  }, [value])
  ...
  return (
    <textarea
      ...
      onChange={(e) => {
        ...
        setLocalValue(v)   // UI darhol
        onChange(id, v)    // parent state
      }}
    />
  )
})
...
export default function WritingTestPage() {
  ...
  // ===== IMPORTANT: global pointer up/cancel (splitter stuck bo‘lmasin) =====
  useEffect(() => {
    const stop = () => {
      setDragging(false)
      setMDragging(false)
  ...
  const doSubmit = useCallback(
    ...
    [examId, isSubmitting, storageKey, startedKey, timeKey, uiKey, tasks, responses, router, getLimits, taskLabel]
  )
  ...
  const mobileQuestionHeight = mExpand === 'question' ? '72vh' : mExpand === 'answer' ? '18vh' : `${mQHeight}px`
  ...
  const AnswerSheet = ({ compact }: { compact?: boolean }) => (
    <div className="space-y-10 pb-24">
      {tasks.map((task) => {
        ...
        const isOk = text.trim().length >= 10 && words >= min && words <= max
        ...
        return (
          ...
          >
            ...
            <TaskTextArea
              ...
              onChange={(taskId, val) => {
                setResponses((prev) => ({ ...prev, [taskId]: val }))
                setVisibleTaskId(taskId)
              }}
            />
