...
export default function WritingListPage() {
  ...
  // --- STATE ---
  const [exams, setExams] = useState<any[]>([]) // Backenddan keladigan snake_case ma'lumotlar uchun
  ...
  useEffect(() => {
    const fetchExams = async () => {
      try {
        ...
        // API response.data formatiga qarab tekshiring
        ...
        >
          <div className="relative z-10">
            ...
            <h2 className="text-3xl md:text-4xl font-black mb-4 tracking-tight leading-tight uppercase">
              Yozish Qismi
              <br /> <span className="italic text-orange-100">Mashqlari</span>
            </h2>
            <p className="text-orange-50 text-sm font-medium max-w-md opacity-90 leading-relaxed">
              Mavzuni tanlang va yozishni boshlang. AI yordamida natijangizni darhol
              tekshiring.
            </p>
          </div>
          <Edit3
            size={180}
            className="absolute -right-10 -bottom-10 text-white opacity-10 -rotate-12"
          />
        </motion.div>
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-black text-slate-800 text-xs tracking-[0.2em] uppercase">
              Mavjud Mavzular
            </h3>
          </div>
          <Badge
            variant="secondary"
            className="bg-orange-50 text-orange-600 rounded-full px-4 py-1 border-none font-black text-[10px]"
          >
        ...
        <div className="grid grid-cols-1 gap-4 pb-10">
          <AnimatePresence mode="popLayout">
            {filteredTests.map((test, index) => {
              // MUHIM: Backend JSON dagi is_free kalitidan foydalanamiz
              ...
              return (
                <motion.div
                  ...
                  className={`group flex flex-col sm:flex-row items-center justify-between p-6 rounded-[32px] bg-white border border-slate-100 hover:border-orange-300 hover:shadow-2xl hover:shadow-orange-500/[0.06] transition-all cursor-pointer relative overflow-hidden 
                    ${!test.is_active ? 'opacity-70 grayscale pointer-events-none' : ''}`}
                >
                  <div className="flex items-center gap-6 z-10 w-full sm:w-auto">
                    <div
                      className={`w-16 h-16 rounded-[24px] flex items-center justify-center transition-all duration-500 shadow-inner shrink-0
                      ${
                        isLocked
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 mb-2">
                        ...
                        {test.is_free ? (
                          <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 text-[9px] font-black uppercase tracking-tighter">
                            OPEN
                          </Badge>
                        ) : (
                          <Badge className="bg-orange-50 text-orange-600 border-orange-100 text-[9px] font-black uppercase tracking-tighter">
                            PREMIUM
                          </Badge>
                        )}
                        <Badge
                          variant="outline"
                          className="text-[9px] font-black border-slate-200"
                        >
                          {test.tasks?.length > 1
                            ? 'Full Test'
                            : `Task ${test.tasks?.[0]?.part_number || 1}`}
                        </Badge>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-5 gap-y-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md">
                          ...
                          <span>{test.tasks?.length || 0} Task(s)</span>
                        </div>
                  ...
                  <div
                    className={`mt-4 sm:mt-0 shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 shadow-sm
                    ${
                      isLocked
