...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    // 2. FAQAT LOGIN TEKSHIRUVI (Raqam uchun redirect olib tashlandi)
    useEffect(() => {
        if (!authLoading && !user) {
    ...
    // 3. MENYU ELEMENTLARI (Barchasi 'disabled: false')
    const menuItems = [
            ...
            <div className="flex-1 flex flex-col lg:ml-72 min-w-0 pb-24 lg:pb-0">
                ...
                <header className="h-16 lg:h-24 px-4 lg:px-12 sticky top-0 bg-white/80 backdrop-blur-md z-30 border-b border-slate-100/50 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        {/* Headerdagi kichik ogohlantirish */}
                        {!isVerifiedUser && (
                            <Link href="/dashboard/profile" className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-full text-amber-700 hover:bg-amber-100 transition-all">
                                <AlertTriangle size={14} className="animate-pulse" />
                                <span className="text-[10px] lg:text-xs font-bold uppercase tracking-wider">Tasdiqlanmagan</span>
                            </Link>
                        )}
                ...
                <main className="p-4 lg:p-12">
                    {/* BARCHA SAHIFALARDA KO'RINADIGAN SOFT BANNER */}
                    <AnimatePresence>
                        {!isVerifiedUser && pathname !== "/dashboard/profile" && (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-black text-slate-800 tracking-tight">Profilni to'ldiring</h4>
                                        <p className="text-xs text-slate-500 mt-0.5">Telefon raqamingiz tasdiqlanmagan. To'liq xavfsizlik uchun raqamni ulang.</p>
                                    </div>
                                </div>
                                <Link href="/dashboard/profile" className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-amber-500/20">
                                    Tasdiqlash
                                </Link>
                            </motion.div>
                        )}
            ...
            <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 px-2 pb-safe pt-2 z-50 shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
                <div className="flex items-center justify-around max-w-md mx-auto">
                    {menuItems.map((item) => {
                        const isActive = pathname.startsWith(item.href)
                        return (
                            <Link key={item.href} href={item.href} className={`flex flex-col items-center gap-1 px-4 py-2 rounded-xl transition-all ${isActive ? "text-[#17776A]" : "text-slate-400"}`}>
                                <item.icon size={22} strokeWidth={isActive ? 2.5 : 2} />
                                <span className="text-[10px] font-bold tracking-tight">{item.name}</span>
                                {isActive && <motion.div layoutId="bottomTab" className="w-1 h-1 bg-[#17776A] rounded-full mt-0.5" />}
                            </Link>
                        )
                    })}
                </div>
