...
import { CheckCircle2, Folder, Headphones, ArrowLeft, ArrowRight, Trash2, Edit } from "lucide-react"
...
export default function ListeningExamContent({
    ...
    // --- 1. BO'LIM TO'LIQ BAJARILGANINI TEKSHIRISH ---
    const isPartDone = useMemo(() => (partIdx: number) => {
        if (!examData?.parts[partIdx]) return false;
        const partQuestions = examData.parts[partIdx].questions;
        return partQuestions.every(q => answers[String(q.id)] && answers[String(q.id)].trim() !== "");
    }, [examData, answers]);
    ...
    const renderGapFillPassage = (text: string, questions: ListeningQuestion[]) => {
        ...
        return (
            <div className="leading-[2.8] text-justify text-slate-800 bg-white p-5 md:p-10 rounded-xl border border-slate-200 shadow-sm mb-10"
    ...
    return (
        <div className="flex flex-col h-full bg-[#F8FAFC]">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-black transition-all duration-300
                                    ${active ? "bg-blue-600 text-white shadow-lg scale-105" :
                                        done ? "bg-emerald-500 text-white" : "text-slate-400 hover:bg-slate-200"}`}
                            >
            ...
            <audio
                ref={audioRef}
                onTimeUpdate={(e) => status === "playing" && onTimeUpdate(e.currentTarget.currentTime, e.currentTarget.duration)}
                onEnded={() => {
            ...
            {/* MAIN CONTENT AREA */}
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-[#F1F5F9]/50 flex flex-col md:flex-row gap-6 relative">
                    ...
                    <div className="w-full md:w-1/2 h-auto bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden md:sticky md:top-0 self-start">
                        <div className="p-3 md:p-4 flex flex-col items-center">
                            ...
                            <img
                                ...
                                alt="Exam Visual"
                                className="w-full h-auto max-h-[350px] md:max-h-[75vh] object-contain rounded-lg"
                                loading="eager"
                            />
                        </div>
                    </div>
                    ...
                    <div className={`grid gap-6 ${isFullWidthPassage || isMapLayout ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2"}`}>
                        {isFullWidthPassage ? (
                            ...
                            currentPart.questions.map((q) => {
                                ...
                                // --- AGAR SAVOL MATCHING BO'LSA (SPEAKERLAR UCHUN) ---
                                if (q.type === 'MATCHING') {
                                    return (
                                        <MatchingDropdown
                                            key={qId}
                                ...
                                // --- MULTIPLE CHOICE YOKI BOSHQALAR UCHUN (ESKI KO'RINISH) ---
                                ...
                                return (
                                    <div
                                        ...
                                        className={`p-4 md:p-6 rounded-2xl border-2 transition-all bg-white cursor-pointer
                        ${isActive ? "border-blue-500 ring-4 ring-blue-500/5 shadow-md" : "border-transparent hover:border-slate-200"}`}
                                    >
                            ...
                            ${isActive ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400"}`}>
                                            ...
                                            <div className="flex-1" onClick={e => e.stopPropagation()}>
                                                {q.question && (
                                                    <p className="font-bold text-slate-800 mb-4 text-sm md:text-base" dangerouslySetInnerHTML={{ __html: q.question }} />
                                                )}
                                            ...
                                            ${answers[qId] === opt.value ? 'bg-blue-50 border-blue-500' : 'border-slate-100 hover:bg-slate-50'}`}>
                                                                <input
                                                                    type="radio"
                                                ...
                                                ${answers[qId] === opt.value ? "border-blue-600 bg-blue-600" : "border-slate-300"}`}>
                                                                    {answers[qId] === opt.value && <div className="w-1.5 h-1.5 bg-white rounded-full"></div>}
                                                                </div>
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none focus:border-blue-500 font-bold"
                                                    />
                                                )}
                                            </div>
                                        </div>
